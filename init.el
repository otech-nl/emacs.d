...
(defconst steets:src-root "c:/Users/steets/src/"
  "Directory with all my sources")
(defconst steets:drive-root
 (if (eq system-type 'windows-nt)
     "C:/Users/steets/Google/rsmetix"
   "~/gdrive"))

(require 'org)  ;; make org-directory available

;; handle config files
(let ((config-files
       (mapcar
        (lambda (f) (expand-file-name (concat f ".el") user-emacs-directory))
        (list "custom" "functions" "hooks" "org" "packages" "keys"))))
