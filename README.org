...
Ad 1: This means I try to learn vanilla key binding, so no UAC for example. Also, I try to work with built-in functionality, before I install a package (less packages, means bugs and conflicts are less likely). Also, I use a built-in theme.
